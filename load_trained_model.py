...
def predict_segmentation(image_path, model, target_size=(128, 128)):
    img_array, original_img_gray = load_preprocess_image(image_path, target_size)
    original_img_rgb = load_img(image_path, color_mode='rgb')
    prediction = model.predict(np.expand_dims(img_array, axis=0))
    predicted_mask = prediction[0, :, :, 0]
    predicted_mask_resized = Image.fromarray((predicted_mask * 255).astype('uint8'))
    predicted_mask_resized = predicted_mask_resized.resize(original_img_gray.size, Image.NEAREST)
    return original_img_rgb, original_img_gray, predicted_mask_resized
...
def all_images(image_paths, model, base_images_path, base_masks_path):
    ...
    for i, image_name in enumerate(image_paths):
        ...
        original_img_rgb, original_img_gray, predicted_mask = predict_segmentation(full_image_path, model)
    ...
    all_images_path = all_images_path = os.path.join(script_dir, 'selected_images.png')
    plt.tight_layout()
